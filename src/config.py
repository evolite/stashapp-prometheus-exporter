...
Environment variables:
    ...
    SCENE_PAGE_SIZE         – Number of scenes per page for pagination
                              (default: 1000, -1 disables pagination)
    SCENE_MAX_SCENES        – Maximum total scenes to process
...
def load_config() -> Config:
    ...
    if scene_page_size != -1 and scene_page_size <= 0:
        raise ValueError("SCENE_PAGE_SIZE must be positive or -1 to disable pagination")

    scene_max_scenes = _get_env_int("SCENE_MAX_SCENES", -1)
    if scene_max_scenes != -1 and scene_max_scenes <= 0:
        raise ValueError("SCENE_MAX_SCENES must be positive or -1 for unlimited")
